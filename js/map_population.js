function map_population (element, dataset, year){
  ...
  queue()
	...
	var pop_medalhas = d3.map();
	var pop_medalhas_max = 0;
	...
	function ready(error, world, medalhasData) {
	  ...
	  for (i in nested) {
	  	x = nested[i].values[0]["Population/Medals"];
	    pop_medalhas.set(nested[i].key, x);
	    if (x > pop_medalhas_max) pop_medalhas_max = x;
	  }
	    ...
	    .html(function(d) {
	    	...
	    	if (pais.length > 0) {
		    	return "<strong>Country:</strong>" + " " + pais[0].Country + 
					"<br>"  + "<strong>" + "Population/medals:</strong> " + 
					nFormatter(pop_medalhas.get(d.id)) + "<br>"; 
			} else {
				return "";
			}
	});
	...
	coef = 255 / Math.log(pop_medalhas_max/20);
	  ...
	  svg.selectAll(".country")
	      ...
	      .style("fill", function(d, i) { 
	          var color = 0;
	          var n = pop_medalhas.get(d.id);
			  //console.log(n);
	          if (n == undefined || n == 0) color = 255;
	          else color = parseInt(255 - coef * Math.log(n/20));
	          /*
