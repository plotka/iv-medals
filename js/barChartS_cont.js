function medals_sports_cont(element, full_dataset, year, continent){
	if (element.clientWidth <=0 ) return;
	if (!continent) return;
	//Filtrar por ano e por país
	var country_dataset = full_dataset.filter(function (x) { 
		return x.Continent == continent  &&  x.Olympics == year;
	});
	...
	var margin = {top: 20, right: 0, bottom: 0, left: 115},
    	width = element.clientWidth/1.05 - margin.left - margin.right,
